...
stations="('PROF_CLYM','PROF_OWEG','PROF_STAT','PROF_STON','PROF_QUEE','PROF_SUFF','PROF_BUFF','PROF_BELL','PROF_TUPP')"
ranges=(
            ...
            for transformed in "not_transformed"; do
                for loss in "${losses[@]}"; do
                    for Ens in $(seq 0 9); do
                        ...
                        python TabNet_multioutput_profilers.py "$stations" "$hourly_data_method" "$range" "$segregated" "$transformed" "$loss" "$Ens" "$gpu_device" &
